...
public class Board
{
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        this.m_traySize = gameSettings.TraySize;
        m_cells = new Cell[boardSizeX, boardSizeY];

        m_trayCells = new List<Cell>();
        CreateBoard();
        CreateTray();
    }

    private void CreateBoard() //tao board
    {
    ...
    private void CreateTray() // create tray
    {
        Vector3 origin = new Vector3(-boardSizeX * 0.5f + 0f, -boardSizeY * 0.5f - 1f, 0f);
        GameObject prefabTray = Resources.Load<GameObject>("prefabs/trayCell");
        for (int i = 0; i < m_traySize; i++)
        {
            GameObject go = GameObject.Instantiate(prefabTray);
            go.transform.position = origin + new Vector3(i * 1f, 0f, 0f);
            go.transform.SetParent(m_root);
            Cell cell = go.GetComponent<Cell>();
            cell.Setup(i, -1);
            m_trayCells.Add(cell);
        }
    }
    public Cell GetEmptyTrayCell()
    {
        return m_trayCells.FirstOrDefault(c => c.IsEmpty);
    }
    public bool CheckTrayMatch()
    {
        var groups = m_trayCells
            .Where(c => !c.IsEmpty)
            .GroupBy(c => GetItemIdentifier(c.Item))
            .Where(g => g.Count() >= 3);
    ...
    private string GetItemIdentifier(Item item)
    {
        ...
        {
            return "Normal_" + normalItem.ItemType.ToString();
        }
        return item.GetType().Name;
    }
    public bool IsTrayFull() => m_trayCells.All(c => !c.IsEmpty);
    internal void Fill()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
